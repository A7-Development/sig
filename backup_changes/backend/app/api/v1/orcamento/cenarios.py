...
from app.db.models.orcamento import Cenario, Premissa, QuadroPessoal, CenarioEmpresa, FuncaoSpan, PremissaFuncaoMes, CenarioCliente, CenarioSecao
...
async def list_cenario_clientes(
    cenario_id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """Lista clientes associados a um cenário."""
    result = await db.execute(
        select(CenarioCliente)
        .options(selectinload(CenarioCliente.secoes).selectinload(CenarioSecao.secao))
        .where(CenarioCliente.cenario_id == cenario_id, CenarioCliente.ativo == True)
        .order_by(CenarioCliente.nome_cliente)
    )
    return result.scalars().all()


@router.post("/{cenario_id}/clientes", response_model=CenarioClienteResponse)
async def create_cenario_cliente(
    cenario_id: UUID,
    ...
    # Verificar se cliente já está no cenário
    existing = await db.execute(
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Cliente já está associado a este cenário")
    
    # Buscar nome do cliente no NW
    nome_cliente = data.nome_cliente
    if not nome_cliente:
        try:
            from app.services.nw import buscar_cliente_nw_por_codigo
            cliente_nw = buscar_cliente_nw_por_codigo(data.cliente_nw_codigo)
            if cliente_nw:
                nome_cliente = cliente_nw.razao_social
        except Exception:
            pass
...
@router.put("/{cenario_id}/clientes/{cliente_id}", response_model=CenarioClienteResponse)
async def update_cenario_cliente(
    cenario_id: UUID,
    cliente_id: UUID,
    data: CenarioClienteUpdate,
    db: AsyncSession = Depends(get_db)
):
    """Atualiza um cliente do cenário."""
    result = await db.execute(
        select(CenarioCliente).where(
...
async def delete_cenario_cliente(
    cenario_id: UUID,
    cliente_id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """Remove um cliente do cenário."""
    result = await db.execute(
        select(CenarioCliente).where(
            CenarioCliente.id == cliente_id,
            CenarioCliente.cenario_id == cenario_id
...
# ============================================
# SEÇÕES DO CLIENTE NO CENÁRIO
# ============================================

@router.get("/{cenario_id}/clientes/{cliente_id}/secoes", response_model=List[CenarioSecaoResponse])
async def list_cenario_secoes(
    cenario_id: UUID,
    cliente_id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """Lista seções de um cliente no cenário."""
    # Verificar se cliente pertence ao cenário
    ...
    result = await db.execute(
        ...
        .where(CenarioSecao.cenario_cliente_id == cliente_id, CenarioSecao.ativo == True)
        .order_by(CenarioSecao.created_at)
    )
    return result.scalars().all()


@router.post("/{cenario_id}/clientes/{cliente_id}/secoes", response_model=CenarioSecaoResponse)
async def create_cenario_secao(
    cenario_id: UUID,
    cliente_id: UUID,
    data: CenarioSecaoCreate,
    db: AsyncSession = Depends(get_db)
):
    """Adiciona uma seção a um cliente no cenário."""
    # Verificar se cliente pertence ao cenário
    cliente_result = await db.execute(
        select(CenarioCliente).where(
            CenarioCliente.id == cliente_id,
    ...
    from app.db.models.orcamento import Secao
    ...
    # Verificar se seção já está no cliente
    existing = await db.execute(
    ...
    # Carregar relacionamento secao
    result = await db.execute(
        select(CenarioSecao)
        .options(selectinload(CenarioSecao.secao))
        .where(CenarioSecao.id == cenario_secao.id)
    )
    return result.scalar_one()


@router.put("/{cenario_id}/clientes/{cliente_id}/secoes/{secao_id}", response_model=CenarioSecaoResponse)
async def update_cenario_secao(
    cenario_id: UUID,
    cliente_id: UUID,
    secao_id: UUID,
    data: CenarioSecaoUpdate,
    db: AsyncSession = Depends(get_db)
):
    """Atualiza uma seção do cliente no cenário."""
    # Verificar se cliente pertence ao cenário
    cliente_result = await db.execute(
        select(CenarioCliente).where(
            CenarioCliente.id == cliente_id,
    ...
    if not cenario_secao:
        raise HTTPException(status_code=404, detail="Seção não encontrada no cliente")
    
    update_data = data.model_dump(exclude_unset=True)
    for key, value in update_data.items():
        setattr(cenario_secao, key, value)
...
async def delete_cenario_secao(
    cenario_id: UUID,
    cliente_id: UUID,
    secao_id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """Remove uma seção do cliente no cenário."""
    # Verificar se cliente pertence ao cenário
    cliente_result = await db.execute(
        select(CenarioCliente).where(
            CenarioCliente.id == cliente_id,
